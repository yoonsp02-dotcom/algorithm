...
int main() {
    ...
    while (C--) {
        int n, q;
        cin >> n >> q;
        vector<int> minSigns(n);
        vector<int> maxSigns(n);
        for (int i = 0; i < n; ++i) {
            cin >> minSigns[i];
            maxSigns[i] = -minSigns[i];
        }

        RMQ minLevel(minSigns);
        RMQ maxLevel(maxSigns);
        for (int i = 0; i < q; ++i) {
            int left, right;
            cin >> left >> right;
            cout << -maxLevel.query(left, right) - minLevel.query(left, right)
                 << "\n";
        }
    }
