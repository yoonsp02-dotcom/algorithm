...
const vector<vector<vector<int>>> cctv = {
    ...
    {{LEFT, UP, RIGHT}, {UP, RIGHT, DOWN}, {RIGHT, DOWN, LEFT}, {DOWN, LEFT, UP}}, // 4
    {{UP, RIGHT, DOWN, LEFT}} // 5번: 네 방향 모두
};
int N, M;
vector<vector<int>> office;

int getMinBlindSpot(vector<vector<int>>& isSurveilled, queue<pair<int, int>> &cctvLoc);
...
int main() {
    ...
    cout << getMinBlindSpot(isSurveilled, cctvLoc) << "\n";
...
void addSurveillArea(vector<vector<int>> &isSurveilled, int y, int x, int dir, int delta) {
...
int getMinBlindSpot(vector<vector<int>> &isSurveilled, queue<pair<int, int>> &cctvLoc) {
    ...
    int y = cctvLoc.front().first;
    int x = cctvLoc.front().second;
    ...
    for (int rot = 0; rot < cctv[cctvNum].size(); ++rot) {
        ...
        // ret = min(ret, getMinBlindSpot(isSurveilled, cctvLoc));
        int tmp = getMinBlindSpot(isSurveilled, cctvLoc);
        if (tmp < ret) {
            ret = tmp;
            cout << "y: " << y << " x: " << x << endl;
            cout << "cctvNum: " << cctvNum << " rot: " << rot << endl;
        }
        for (int i = 0; i < cctv[cctvNum][rot].size(); ++i) {
            int dir = cctv[cctvNum][rot][i];
            addSurveillArea(isSurveilled, y, x, dir, -1);
